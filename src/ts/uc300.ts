...
function decode(bytes: number[]) {
  ...
  while (reader.hasNext()) {
    ...
    for (let i = 0; i < quantity; i++) {
      let mBytes: number = reader.read(dataSize);
 ...
 * sub function
 ********************************************/
const getDataType = (bytes: number[]) => {
  return byte2hex(bytes[0]);
};

const getPacketLength = (bytes: number[]) => {
  return readInt16LE(bytes);
};

const getPacketVersion = (bytes: number[]) => {
  return readInt8LE(bytes);
};

const getTimeStamp = (bytes) => {
  return new Date(readUInt32LE(bytes) * 1000).toString();
};

const getSignalStrength = (bytes) => {
  return readInt8LE(bytes);
};
